...
interface GroupStats {
  totalMembers: number;
  activeMembers: number; // 최근 7일 내 활동
...
export default function GroupAdminPage() {
  ...
  const isChurchGroup = !!group?.church_id;
  ...
  const loadData = useCallback(async () => {
    ...
    if (groupData.church_id) {
      ...
      if (church) {
        ...
        // 현재 사용자가 교회 관리자인지 확인 (localStorage 토큰 비교)
        const storedToken = typeof window !== 'undefined'
          ? localStorage.getItem(`church_admin_${church.code}`)
          : null;
        if (storedToken && church.admin_token === storedToken) {
  ...
  const inactiveMembers = members.filter(m => {
    const memberInActiveSet = stats.activeMembers > 0;
    return !memberInActiveSet && m.readDays === 0 && m.commentsCount === 0;
  });
  ...
  return (
    <div className="flex flex-col p-4 space-y-4 pb-20">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {group.name}
              {churchInfo && (
                <span className="ml-1">· {churchInfo.name}</span>
              )}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <Link href={`/group/${groupId}/admin/ranks`}>
      ...
      {/* 교회 그룹 권한 안내 */}
      {isChurchGroup && !isChurchAdmin && (
        <Card className="border-accent/30 bg-accent/10/50">
          <CardContent className="pt-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-accent flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-foreground">교회 연동 그룹</p>
                <p className="text-sm text-accent mt-1">
                  이 그룹은 {churchInfo?.name}에 연동되어 있습니다.
                  게시글 삭제 권한은 교회 관리자만 가집니다.
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card className={cn(
          stats.activeMembers < stats.totalMembers / 2 && "border-border bg-muted"
        )}>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-1">
          ...
          {members.map((member, index) => {
            ...
            return (
              <div
                ...
                onClick={() => router.push(`/group/${groupId}/admin/member/${member.user_id}`)}
              >
